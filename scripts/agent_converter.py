...
def _indented_increase_indent(
    self: yaml.Dumper, flow: bool = False, indentless: bool = False
) -> None:
...
class FolderToJsonConverter:
    ...
    async def _parse_skills(
        self, agent_dir: Path, skill_paths: list[str]
    ) -> list[dict]:
        skills = []
    ...
    async def _parse_subagents(
        self, agent_dir: Path, subagent_paths: list[str]
    ) -> list[dict]:
        subagents = []
...
class JsonToFolderConverter:
    ...
    async def _write_skills(
        self, skills: list[dict], skills_base_dir: Path
    ) -> list[str]:
        paths: list[str] = []
        for skill in skills:
    ...
    async def _write_subagents(
        self, subagents: list[dict], subagents_base_dir: Path
    ) -> list[str]:
        paths: list[str] = []
...
async def main() -> None:
    ...
    p_json.add_argument(
        "agent_name", help="Name of the agent folder (e.g. 'sales-agent')"
    )
    p_json.add_argument(
        "--dir",
